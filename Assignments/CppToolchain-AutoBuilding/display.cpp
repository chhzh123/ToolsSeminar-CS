...
void keyPressed(unsigned char key, int x, int y)
{
    ...
    case '2':
        mode = 2;
        printf("* Change to flat mode");
        break;
    case '3':
        mode = 3;
        printf("* Change to flat lines");
        break;
    }
...
#ifdef TEST1
    model.loadOBJ("test1.in");
#elif TEST2
    model.loadOFF("test2.in");
#elif TEST3
    model.loadPLY("test3.in");
#endif

    if (model.fmode == 0){
        printf("Error: You do not load any models!\n");
        return 0;
    }

    printf("You correctly run the program!\n");
